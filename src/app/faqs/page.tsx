      ...
      <main className="flex-grow pt-[72px] md:pt-20">
        ...
        <SectionWrapper theme="charius-beige">
          <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
            {faqs.map((item) => (
              <PremiumCard key={item.question} variant="charius" className="p-8 bg-white" withHover>
                <h3 className="text-xl font-bold text-[#111] mb-4">{item.question}</h3>
                <p className="text-gray-500 leading-relaxed text-lg">{item.answer}</p>
              </PremiumCard>
            ))}
          </div>
        </SectionWrapper>
