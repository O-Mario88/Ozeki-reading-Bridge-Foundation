...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
    ...
    // Sub-counties are formatted as "District::SubCounty" in the API, let's extract the name
    const displayName = name.split("::").pop() || name;

    return {
        title: `${displayName} Sub-County — Impact Profile | Ozeki Reading Bridge Foundation`,
        description: `Sub-County level literacy implementation data for ${displayName} — fidelity scores, learning gains, and school coverage.`,
        openGraph: {
            title: `${displayName} Sub-County — Impact Profile`,
            description: `Sub-County level literacy data for ${displayName}.`,
            type: "website",
        },
    };
}

export default async function SubCountyPage({ params }: { params: Params }) {
    const { id } = await params;
    const name = decodeURIComponent(id);
    const displayName = name.split("::").pop() || name;
            ...
            <section className="page-hero" style={{ backgroundImage: "url('/photos/PXL_20260217_124357146.jpg')" }}>
                <div className="container">
                    <nav className="impact-dash-breadcrumb" aria-label="Drill-down">
                        ...
                        <span aria-hidden>›</span>
                        <span>{displayName} Sub-County</span>
                    </nav>
                    <p className="kicker">Sub-County Profile</p>
                    <h1>{displayName}</h1>
                    <p>Implementation and outcome data for {displayName} sub-county in {districtName} district.</p>
                </div>
            </section>
            ...
            <section className="section" style={{ backgroundColor: "var(--md-sys-color-surface-container-low, #f6f6f6)" }}>
                <div className="container" style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
                    ...
                    <p className="note-box impact-compliance-note">
                        This page shows aggregated sub-county-level data only.
                    </p>
                </div>
            </section>
        </>
