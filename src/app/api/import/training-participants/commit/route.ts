...
import { commitTrainingParticipantsImport } from "@/lib/server/imports/training-participants";
...
export async function POST(request: Request) {
  ...
  return withRouteHandler({
    route: "/api/import/training-participants/commit",
    method: "POST",
    userId: user?.id ?? null,
    handler: async ({ requestId }) => {
      ...
      const result = await commitTrainingParticipantsImport({
        actor: user,
