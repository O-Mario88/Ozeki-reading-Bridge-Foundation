...
export async function POST(request: Request) {
  ...
  try {
    ...
    const baseDir = path.join(process.cwd(), "public", "uploads", "stories", year, month);
    await fs.mkdir(baseDir, { recursive: true });

    const baseName = sanitizeSegment(path.basename(file.name, path.extname(file.name))).slice(0, 60) || "story";
    ...
    const publicUrl = `/uploads/stories/${year}/${month}/${storedFile}`;
