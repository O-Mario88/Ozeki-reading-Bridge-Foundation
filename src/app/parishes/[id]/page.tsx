...
export async function generateMetadata({ params }: { params: Params }): Promise<Metadata> {
    ...
    // Parishes are formatted as "District::SubCounty::Parish" in the API
    const parts = name.split("::");
    const displayName = parts[parts.length - 1] || name;

    return {
        title: `${displayName} Parish — Impact Profile | Ozeki Reading Bridge Foundation`,
        description: `Parish level literacy implementation data for ${displayName} — fidelity scores, learning gains, and school coverage.`,
        openGraph: {
            title: `${displayName} Parish — Impact Profile`,
            description: `Parish level literacy data for ${displayName}.`,
            type: "website",
        },
    };
}

export default async function ParishPage({ params }: { params: Params }) {
    const { id } = await params;
    const name = decodeURIComponent(id);
    const parts = name.split("::");
    const displayName = parts[parts.length - 1] || name;
            ...
            <section className="page-hero" style={{ backgroundImage: "url('/photos/PXL_20260217_110739688.jpg')" }}>
                <div className="container">
                    <nav className="impact-dash-breadcrumb" aria-label="Drill-down">
                        ...
                        <Link href={`/districts/${encodeURIComponent(districtName)}`}>{districtName}</Link>
                        <span aria-hidden>›</span>
                        <Link href={`/sub-counties/${encodeURIComponent(districtName + '::' + subCountyName)}`}>{subCountyName}</Link>
                        <span aria-hidden>›</span>
                        <span>{displayName} Parish</span>
                    </nav>
                    <p className="kicker">Parish Profile</p>
                    <h1>{displayName}</h1>
                    <p>Implementation and outcome data for {displayName} parish in {subCountyName} sub-county, {districtName} district.</p>
                </div>
            </section>
                ...
                <div className="container" style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
                    ...
                    <div className="impact-dash-kpi-grid">
                        ...
                        {gains.schoolImprovementIndex !== null && (
                            <article className="impact-dash-kpi" style={{
                                "--kpi-accent": gains.schoolImprovementIndex > 0 ? "#ff7235" : "#dc2626",
                            } as React.CSSProperties}>
                                <div className="impact-dash-kpi-body">
                        ...
                        {fidelity.drivers.map((d) => (
                            <div key={d.driver} style={{ marginBottom: "0.75rem" }}>
                                <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", fontSize: "0.85rem" }}>
                                    ...
                                    <span style={{ fontWeight: 700, color: d.score >= 75 ? "#ff7235" : d.score >= 50 ? "#e8a317" : "#dc2626" }}>{d.score}%</span>
                                </div>
                                <div style={{ height: "6px", borderRadius: "4px", background: "#e8e8e8", overflow: "hidden", marginTop: "0.15rem" }}>
                                    <div style={{ width: `${d.score}%`, height: "100%", borderRadius: "4px", background: d.score >= 75 ? "#ff7235" : d.score >= 50 ? "#e8a317" : "#dc2626" }} />
                                </div>
                    ...
                    <p className="note-box impact-compliance-note">
                        This page shows aggregated parish-level data only. No individual learner information is shown.
                    </p>
                </div>
            </section>
        </>
