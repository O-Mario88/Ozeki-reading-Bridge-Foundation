...
import { FileText, Download, Users, Lightbulb, PlayCircle, Loader2, BarChart3, Film } from "lucide-react";
import { LiveAttendanceDashboard } from "./LiveAttendanceDashboard";
import { SessionReplayPlayer } from "./SessionReplayPlayer";
import { SessionEffectivenessPanel } from "./SessionEffectivenessPanel";
...
function normalizeResource(value: unknown): SessionResourceViewModel {
  ...
  return {
    ...
    fileUrl:
      ...
      typeof record.stored_path === "string" ? record.stored_path : null,
  };
}
...
export function SessionTabs({ session, resources, artifacts, isStaff, isHost = false }: SessionTabsProps) {
  ...
  const recordingUrl = session.recordingUrl ?? session.recording_url ??
    normalizedArtifacts.find((a) => a.type === "recording" && a.status === "available")?.sourceUrl ?? null;
  ...
  const tabs = [
    ...
    { id: "replay", label: "Replay", icon: Film, show: !!recordingUrl || isCompleted },
  ...
  if (isStaff) {
    tabs.push({ id: "attendance", label: "Attendance", icon: Users, show: true });
    if (isCompleted) {
      tabs.push({ id: "effectiveness", label: "Effectiveness", icon: BarChart3, show: true });
    }
  }
      ...
      <div className="p-6 md:p-8 flex-1 bg-white">
        ...
        {activeTab === "description" && (
          <div className="animate-in fade-in duration-300">
            ...
            <div className="prose prose-blue prose-sm max-w-none text-gray-600 whitespace-pre-wrap">
              {session.agenda}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-5 rounded-xl border border-gray-200 bg-gray-50 flex items-center justify-between">
                ...
                {hasRecording && recordingUrl && (
                  <a href={recordingUrl} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors">
                    Open Video
                  </a>
                )}
              </div>
        ...
        {activeTab === "attendance" && isStaff && sessionId && (
          <div className="animate-in fade-in duration-300">
            <h3 className="text-lg font-bold text-brand-primary mb-6 flex items-center gap-2">
              <Users className="w-5 h-5 text-gray-400" />
              Live Attendance
            </h3>
            <LiveAttendanceDashboard sessionId={sessionId} sessionStatus={session.status ?? "scheduled"} />
          </div>
        )}

        {activeTab === "effectiveness" && isStaff && sessionId && (
          <div className="animate-in fade-in duration-300">
            <h3 className="text-lg font-bold text-brand-primary mb-6 flex items-center gap-2">
              <BarChart3 className="w-5 h-5 text-gray-400" />
              Session Effectiveness
            </h3>
            <SessionEffectivenessPanel sessionId={sessionId} />
          </div>
        )}
